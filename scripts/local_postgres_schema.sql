-- SMS Database Schema for Local PostgreSQL Development
-- Generated from the new normalized ERD design
-- This version excludes Supabase-specific features (RLS, auth.uid(), etc.)
...
('Demo School', 'A sample educational institution for local development', 'admin@demoschool.local');
...
('admin@demoschool.local', 'Admin', 'User', '$2b$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/LewdBPj/SJHYyJhhe', true);

-- Get the user and organization IDs for role assignment
DO $$
DECLARE
    admin_user_id UUID;
    demo_org_id UUID;
BEGIN
    SELECT id INTO admin_user_id FROM users WHERE email = 'admin@demoschool.local';
    SELECT id INTO demo_org_id FROM organizations WHERE email = 'admin@demoschool.local';
    
    -- Assign org_owner role to admin user
    INSERT INTO user_roles (user_id, role, organization_id) 
    VALUES (admin_user_id, 'org_owner', demo_org_id);
    
    -- Update organization owner
    UPDATE organizations SET owner_user_id = admin_user_id WHERE id = demo_org_id;
END $$;
